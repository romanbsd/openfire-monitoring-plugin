...
public class PaginatedMucMessageFromOpenfireOpenSearchQuery extends AbstractPaginatedMamMucQuery
{
    private static final Logger Log = LoggerFactory.getLogger(PaginatedMucMessageFromOpenfireOpenSearchQuery.class);

    public PaginatedMucMessageFromOpenfireOpenSearchQuery(@Nullable final Date startDate, @Nullable final Date endDate, @Nonnull final MUCRoom room, @Nullable final JID with, @Nonnull final String query)
    {
    ...
    protected String getIndexName() {
        return OpenSearchClientHolder.indexName("muc-messages");
    }
    ...
    public List<ArchivedMessage> getPage(final Long after, final Long before, final int maxResults, final boolean isPagingBackwards) throws DataRetrievalException {
        ...
        try {
            ...
            Log.warn("An exception occurred while trying to query OpenSearch to get messages from room {}.", room, e);
    ...
    protected Query getQueryForAllResults() {
        ...
        filters.add(OpenSearchQueryHelper.termQuery("roomID", room.getID()));
        filters.add(OpenSearchQueryHelper.rangeQuery("logTime", startDate.getTime(), endDate.getTime()));
        if (with != null) {
    ...
    public String toString() {
        return "PaginatedMucMessageFromOpenfireOpenSearchQuery{" +
            "startDate=" + startDate +
            ", endDate=" + endDate +
            ", room=" + room +
            ", query='" + query + '\'' +
